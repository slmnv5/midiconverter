...
public:
	MidiFilterRule(const string &ruleFile) : rule_mapper(ruleFile)
	{
	}
...
public:
	MidiFilterCount(const string &fileName) : note_counter(fileName)
	{
	}
	~MidiFilterCount()
	{
	...
	virtual void process_one_event(snd_seq_event_t *event, MidiEvent &ev);
	void send_event_delayed(snd_seq_event_t *event, MidiEvent ev, int c_on,
							int c_off);
};
