...
MidiEventRange::MidiEventRange(const string &s, bool out) : isOut(out), evtype(MidiEventType::ANYTHING)
{
...
{
	ostringstream ss;
	ss << inEventRange.toString() << "=" << outEventRange.toString() << "=" << static_cast<char>(rutype);
	return ss.str();
}
