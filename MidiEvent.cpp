...
//========== free functions ==================
bool
writeMidiEvent(snd_seq_event_t* event, const MidiEvent& ev)
{
...
bool
readMidiEvent(const snd_seq_event_t* event, MidiEvent& ev)
{
...
void
MidiRange<max>::init(const string& s)
{
  string s1(s);
  remove_spaces(s1);
  if (s1.empty())
...
bool
MidiEventRange::match(const MidiEvent& ev) const
{
  if (isOut)
...
void
MidiEventRange::transform(MidiEvent& ev) const
{
  if (!isOut)
