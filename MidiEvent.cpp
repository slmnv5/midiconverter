...
MidiEventRange::MidiEventRange(const string &s, bool out) : isOutEvent(out), evtype(MidiEvType::ANYTHING)
{
...
bool MidiEventRange::match(const MidiEvent &ev) const
{
	...
	return (evtype == ev.evtype || evtype == MidiEvType::ANYTHING) && ev.ch >= ch.lower && ev.ch <= ch.upper && ev.v1 >= v1.lower && ev.v1 <= v1.upper && ev.v2 >= v2.lower && ev.v2 <= v2.upper;
}
